...
class batch_images_to_txt:
    ...
    def integration_unit_conversion(self):
        from grating_function_20190117 import GratingCalculationOnPicture
        self.xlabel = "nm"
    ...
    def calibrate_my_spectrum(self, efficiency_cal_data):
        if len(self.result_binned_array) == len(efficiency_cal_data):
            # print(self.filename)
            self.xlabel = "nm with binsize:  " + str(self.result_binsize) + "nm"
            self.ylabel = "Nphoton"
            self.result_binned_array[:, 1] = self.result_binned_array[:, 1] * efficiency_cal_data[:]
            return self.result_binned_array

        else:
            if len(efficiency_cal_data) > len(self.result_binned_array):
                efficiency_cal_data = efficiency_cal_data[0:-1]
            else:
                self.result_binned_array = self.result_binned_array[0:-1]

            self.calibrate_my_spectrum(efficiency_cal_data)
            return self.result_binned_array

    def convert_to_energy(self):
        #todo check constant for Joule to eV !
...
calibration = GetCalibration(wanted_binsize, x_max, x_min, accuracy_decimal, file_path_list)
...
print(my_picture_list_numbers, 'all?')
...
# np.savetxt("Energy_content_uJ"+'20190117_0.5N25'+".txt",integrated_energy_list, fmt='%.3E', delimiter='\t')
plt.show
